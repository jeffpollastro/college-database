'use client'

import { useState, useEffect } from 'react'
import dynamic from 'next/dynamic'
import { useParams } from 'next/navigation'
import Link from 'next/link'
import { supabase, School } from '@/lib/supabase'
import SiteHeader from '@/components/SiteHeader'
import SiteFooter from '@/components/SiteFooter'

const SchoolMap = dynamic(() => import('@/components/SchoolMap'), { ssr: false })

export default function SchoolDetail() {
  const params = useParams()
  const [school, setSchool] = useState<School | null>(null)
  const [loading, setLoading] = useState(true)
  const [incomeBracket, setIncomeBracket] = useState('0-30k')
  const [isInCompare, setIsInCompare] = useState(false)

  useEffect(() => {
    const stored = localStorage.getItem('compareSchools')
    if (stored && params.id) {
      const ids = JSON.parse(stored) as string[]
      setIsInCompare(ids.includes(params.id as string))
    }
  }, [params.id])

  const toggleCompare = () => {
    const stored = localStorage.getItem('compareSchools')
    let ids = stored ? JSON.parse(stored) as string[] : []
    const schoolId = params.id as string

    if (ids.includes(schoolId)) {
      ids = ids.filter(id => id !== schoolId)
      setIsInCompare(false)
    } else {
      if (ids.length >= 4) {
        alert('You can compare up to 4 schools at a time')
        return
      }
      ids.push(schoolId)
      setIsInCompare(true)
    }
    localStorage.setItem('compareSchools', JSON.stringify(ids))
  }

  useEffect(() => {
    async function fetchSchool() {
      const { data, error } = await supabase
        .from('schools')
        .select('*')
        .eq('id', params.id)
        .single()

      if (error) {
        console.error('Error fetching school:', error)
      } else {
        setSchool(data)
      }
      setLoading(false)
    }

    if (params.id) {
      fetchSchool()
    }
  }, [params.id])

  const formatMoney = (amount: number | null) => {
    if (amount === null || amount === undefined) return 'N/A'
    return '$' + amount.toLocaleString()
  }

  const formatPercent = (value: number | null) => {
    if (value === null || value === undefined) return 'N/A'
    return (value * 100).toFixed(0) + '%'
  }

  const getGapForBracket = (bracket: string): number | null => {
    if (!school) return null
    const gaps: Record<string, number | null> = {
      '0-30k': school.gap_0_30k,
      '30-48k': school.gap_30_48k,
      '48-75k': school.gap_48_75k,
      '75-110k': school.gap_75_110k,
      '110k+': school.gap_110k_plus,
    }
    return gaps[bracket]
  }

  const getSeverityColor = (gap: number | null) => {
    if (gap === null) return 'bg-gray-100 text-gray-800 border-gray-300'
    if (gap < 0) return 'bg-green-100 text-green-800 border-green-300'
    if (gap <= 2500) return 'bg-green-100 text-green-800 border-green-300'
    if (gap <= 7500) return 'bg-yellow-100 text-yellow-800 border-yellow-300'
    if (gap <= 15000) return 'bg-orange-100 text-orange-800 border-orange-300'
    return 'bg-red-100 text-red-800 border-red-300'
  }

  const getGapLabel = (gap: number | null) => {
    if (gap === null) return 'Unknown'
    if (gap < 0) return 'Money Back'
    if (gap === 0) return 'Free'
    if (gap <= 2500) return 'Very Affordable'
    if (gap <= 7500) return 'Moderate'
    if (gap <= 15000) return 'Expensive'
    return 'Very Expensive'
  }

  if (loading) {
    return (
      <main className="min-h-screen bg-cream flex items-center justify-center">
        <div className="text-ink-soft">Loading school details…</div>
      </main>
    )
  }

  if (!school) {
    return (
      <main className="min-h-screen bg-cream flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-heading text-2xl font-bold text-ink mb-4">School Not Found</h1>
          <Link href="/" className="text-brand-dark hover:underline">Back to Search</Link>
        </div>
      </main>
    )
  }

  const currentGap = getGapForBracket(incomeBracket)
  const trueCost = (currentGap || 0) + (school.annual_travel_cost || 0)
  const fourYearGap = currentGap !== null ? currentGap * 4 : null
  const fourYearTrueCost = trueCost * 4

  return (
    <main className="min-h-screen bg-cream">
      <SiteHeader />
      <div className="bg-gradient-to-br from-brand to-brand-dark text-ink py-6 px-4">
        <div className="max-w-4xl mx-auto">
          <Link href="/" className="inline-flex items-center gap-1.5 text-ink/70 hover:text-ink text-sm font-medium mb-4 transition-colors">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <path d="M15 18l-6-6 6-6" />
            </svg>
            Back to Search
          </Link>
          <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
            <div>
              <h1 className="font-heading text-3xl font-bold mb-2">{school.name}</h1>
              <p className="text-ink/80">{school.city}, {school.state}</p>
              <p className="text-ink/70 text-sm mt-1">
                {school.control === 1 ? 'Public' : 'Private'} · {school.size?.toLocaleString()} students
              </p>
            </div>
            <div className="flex flex-col gap-2 items-end">
              {school.no_loan_policy && (
                <span className="bg-green-600 text-white px-3 py-1 rounded-full text-sm font-medium">
                  No-Loan Policy
                </span>
              )}
              {school.hbcu && (
                <span className="bg-ink text-white px-3 py-1 rounded-full text-sm font-medium">
                  HBCU
                </span>
              )}
              <button
                onClick={toggleCompare}
                className={`px-3 py-1 rounded-full text-sm font-medium cursor-pointer transition-colors ${
                  isInCompare
                    ? 'bg-plum text-white hover:bg-plum-light'
                    : 'bg-ink/10 text-ink hover:bg-ink/20'
                }`}
              >
                {isInCompare ? '✓ In Compare List' : '+ Add to Compare'}
              </button>
            </div>
          </div>
        </div>
      </div>

      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <label className="block text-sm font-medium text-ink-soft mb-2">
            Calculate costs for your family income:
          </label>
          <select
            value={incomeBracket}
            onChange={(e) => setIncomeBracket(e.target.value)}
            className="h-11 border border-ink/15 rounded-xl px-4 bg-surface focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand transition-colors"
          >
            <option value="0-30k">$0 - $30,000</option>
            <option value="30-48k">$30,001 - $48,000</option>
            <option value="48-75k">$48,001 - $75,000</option>
            <option value="75-110k">$75,001 - $110,000</option>
            <option value="110k+">$110,001+</option>
          </select>
        </div>

        <div className={'rounded-2xl shadow-sm p-6 mb-6 border-2 ' + getSeverityColor(currentGap)}>
          <div className="text-center">
            <div className="text-sm font-medium mb-1">Your Annual Gap</div>
            <div className="text-4xl font-bold mb-2">
              {currentGap !== null && currentGap < 0
                ? '+' + formatMoney(Math.abs(currentGap))
                : formatMoney(currentGap)
              }
            </div>
            <div className="text-lg font-medium mb-4">{getGapLabel(currentGap)}</div>

            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-current/20">
              <div>
                <div className="text-sm opacity-80">+ Travel from Poconos</div>
                <div className="text-xl font-semibold">{formatMoney(school.annual_travel_cost)}/yr</div>
              </div>
              <div>
                <div className="text-sm opacity-80">= True Annual Cost</div>
                <div className="text-xl font-bold">{formatMoney(trueCost)}/yr</div>
              </div>
            </div>
          </div>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-2">How The Gap Is Calculated</h2>
          <p className="text-sm text-ink-soft mb-4">
            The Gap is what your family actually pays out-of-pocket after all grants and scholarships are applied.
            It&apos;s the difference between the school&apos;s total cost and the financial aid you&apos;re expected to receive.
          </p>

          <div className="bg-cream rounded-xl p-4">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-ink-soft">Published Cost of Attendance</span>
                <span className="font-semibold text-ink">{formatMoney(school.cost_of_attendance)}</span>
              </div>
              <div className="text-xs text-ink-soft -mt-2 mb-2">
                (Tuition, room & board, books, fees, and living expenses)
              </div>

              {currentGap !== null && school.cost_of_attendance && (
                <div className="flex justify-between items-center text-green-700">
                  <span>Estimated Grant Aid for Your Income</span>
                  <span className="font-semibold">− {formatMoney(school.cost_of_attendance - currentGap)}</span>
                </div>
              )}

              <div className="border-t border-ink/15 pt-3 flex justify-between items-center">
                <span className="font-bold text-ink">Your Gap (What You Pay)</span>
                <span className="font-bold text-lg">
                  {currentGap !== null && currentGap < 0
                    ? <span className="text-green-700">+{formatMoney(Math.abs(currentGap))}</span>
                    : formatMoney(currentGap)
                  }
                </span>
              </div>

              {currentGap !== null && currentGap < 0 && (
                <div className="text-sm text-green-700 bg-green-50 p-2 rounded-lg mt-2">
                  <strong>Good news!</strong> At your income level, this school provides more aid than the cost of attendance.
                  The extra {formatMoney(Math.abs(currentGap))} can help cover personal expenses, transportation, or other costs.
                </div>
              )}
            </div>
          </div>

          <div className="mt-4 text-sm text-ink-soft">
            <p><strong className="text-ink">Important:</strong> The Gap does NOT include loans. Loans are money you must pay back,
            so we don&apos;t count them as aid. This number shows what you&apos;d need to cover through savings,
            work-study, or borrowing.</p>
          </div>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">4-Year Cost Projection</h2>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-sm text-ink-soft">4-Year Gap Total</div>
              <div className="text-2xl font-bold text-ink">
                {fourYearGap !== null && fourYearGap < 0
                  ? '+' + formatMoney(Math.abs(fourYearGap))
                  : formatMoney(fourYearGap)
                }
              </div>
            </div>
            <div>
              <div className="text-sm text-ink-soft">4-Year Travel</div>
              <div className="text-2xl font-bold text-ink">{formatMoney((school.annual_travel_cost || 0) * 4)}</div>
            </div>
            <div className="bg-brand/10 rounded-xl p-3">
              <div className="text-sm text-brand-dark">4-Year True Cost</div>
              <div className="text-2xl font-bold text-ink">{formatMoney(fourYearTrueCost)}</div>
            </div>
          </div>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">Gap by Family Income</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink/10">
                  <th className="text-left py-2 text-ink-soft font-medium">Family Income</th>
                  <th className="text-right py-2 text-ink-soft font-medium">Annual Gap</th>
                  <th className="text-right py-2 text-ink-soft font-medium">4-Year Total</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { bracket: '0-30k', label: '$0 - $30,000' },
                  { bracket: '30-48k', label: '$30,001 - $48,000' },
                  { bracket: '48-75k', label: '$48,001 - $75,000' },
                  { bracket: '75-110k', label: '$75,001 - $110,000' },
                  { bracket: '110k+', label: '$110,001+' },
                ].map(({ bracket, label }) => {
                  const gap = getGapForBracket(bracket)
                  const isSelected = bracket === incomeBracket
                  return (
                    <tr key={bracket} className={'border-b border-ink/10 ' + (isSelected ? 'bg-brand/10 font-medium' : '')}>
                      <td className="py-2 text-ink">{label}</td>
                      <td className="text-right py-2 text-ink">
                        {gap !== null && gap < 0
                          ? <span className="text-green-600">+{formatMoney(Math.abs(gap))}</span>
                          : formatMoney(gap)
                        }
                      </td>
                      <td className="text-right py-2 text-ink">
                        {gap !== null && gap < 0
                          ? <span className="text-green-600">+{formatMoney(Math.abs(gap * 4))}</span>
                          : formatMoney(gap !== null ? gap * 4 : null)
                        }
                      </td>
                    </tr>
                  )
                })}
              </tbody>
            </table>
          </div>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">Admissions</h2>
          {school.admission_rate || school.sat_read_25 || school.act_25 ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                {school.admission_rate && (
                  <div className="text-center p-4 bg-cream rounded-xl">
                    <div className="text-2xl font-bold text-ink">{(school.admission_rate * 100).toFixed(0)}%</div>
                    <div className="text-sm text-ink-soft">Admission Rate</div>
                  </div>
                )}
                {school.sat_read_25 && school.sat_math_25 && (
                  <div className="text-center p-4 bg-cream rounded-xl">
                    <div className="text-2xl font-bold text-ink">
                      {school.sat_read_25 + school.sat_math_25} - {school.sat_read_75 + school.sat_math_75}
                    </div>
                    <div className="text-sm text-ink-soft">SAT Range (middle 50%)</div>
                  </div>
                )}
                {school.act_25 && (
                  <div className="text-center p-4 bg-cream rounded-xl">
                    <div className="text-2xl font-bold text-ink">{school.act_25} - {school.act_75}</div>
                    <div className="text-sm text-ink-soft">ACT Range (middle 50%)</div>
                  </div>
                )}
              </div>
              <div className="text-xs text-ink-soft">
                <p>&ldquo;Middle 50%&rdquo; means 25% of admitted students scored below this range and 25% scored above it. Being below the range does not mean automatic rejection.</p>
              </div>
            </>
          ) : (
            <p className="text-ink-soft text-sm">Admission data not reported. This school may have open enrollment or test-optional admissions.</p>
          )}
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">Student Outcomes</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-cream rounded-xl">
              <div className="text-2xl font-bold text-ink">{formatPercent(school.grad_rate_4yr)}</div>
              <div className="text-sm text-ink-soft">4-Year Grad Rate</div>
            </div>
            <div className="text-center p-4 bg-cream rounded-xl">
              <div className="text-2xl font-bold text-ink">{formatPercent(school.grad_rate_pell)}</div>
              <div className="text-sm text-ink-soft">Pell Grad Rate</div>
              <div className="text-xs text-ink-soft">(low-income students)</div>
            </div>
            <div className="text-center p-4 bg-cream rounded-xl">
              <div className="text-2xl font-bold text-ink">{formatMoney(school.median_debt)}</div>
              <div className="text-sm text-ink-soft">Median Debt</div>
            </div>
            <div className="text-center p-4 bg-cream rounded-xl">
              <div className="text-2xl font-bold text-ink">{formatMoney(school.median_earnings_10yr)}</div>
              <div className="text-sm text-ink-soft">Earnings (10yr)</div>
            </div>
          </div>
        </div>

        {school.latitude != null && school.longitude != null && (
          <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-4 mb-6">
            <h2 className="font-heading text-lg font-semibold text-ink mb-3">Location</h2>
            <SchoolMap
              schools={[school]}
              incomeBracket={incomeBracket as '0-30k' | '30-48k' | '48-75k' | '75-110k' | '110k+'}
              height="320px"
              singleSchool
            />
          </div>
        )}

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">Getting There from the Poconos</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="text-sm text-ink-soft">Travel Type</div>
              <div className="text-lg font-medium text-ink">{school.travel_type === 'FLY' ? 'Fly' : 'Drive'}</div>
            </div>
            <div>
              <div className="text-sm text-ink-soft">Annual Travel Budget</div>
              <div className="text-lg font-medium text-ink">{formatMoney(school.annual_travel_cost)}</div>
              <div className="text-xs text-ink-soft">(estimated 5 trips/year)</div>
            </div>
          </div>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-ink/5 p-6 mb-6">
          <h2 className="font-heading text-lg font-semibold text-ink mb-4">Learn More</h2>
          <div className="flex flex-wrap gap-3">
            {school.npc_url && (
              <a
                href={school.npc_url.startsWith('http') ? school.npc_url : 'https://' + school.npc_url}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-brand text-ink font-medium px-4 py-2 rounded-lg hover:bg-brand-dark transition-colors"
              >
                Net Price Calculator
              </a>
            )}
            {school.website_url && (
              <a
                href={school.website_url.startsWith('http') ? school.website_url : 'https://' + school.website_url}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-ink/5 text-ink px-4 py-2 rounded-lg hover:bg-ink/10 transition-colors"
              >
                School Website
              </a>
            )}
          </div>
        </div>

        <div className="bg-gradient-to-br from-ink to-ink-soft rounded-2xl shadow-sm p-6 text-white">
          <h2 className="font-heading text-xl font-bold mb-2">Make This Happen</h2>
          <p className="text-white/80 mb-6">
            Ready to make {school.name} a reality for your family? Here&apos;s your action plan:
          </p>

          <div className="space-y-4">
            <div className="flex items-start gap-4 bg-white/10 rounded-xl p-4">
              <div className="bg-brand text-ink rounded-full w-8 h-8 flex items-center justify-center font-bold flex-shrink-0">1</div>
              <div>
                <h3 className="font-semibold">Get Your Personalized Estimate</h3>
                <p className="text-white/80 text-sm mt-1">
                  Use the school&apos;s Net Price Calculator for a more accurate estimate based on your specific situation.
                  Have your family&apos;s tax info ready.
                </p>
                {school.npc_url && (
                  <a
                    href={school.npc_url.startsWith('http') ? school.npc_url : 'https://' + school.npc_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-block mt-2 bg-brand text-ink px-3 py-1 rounded-lg text-sm font-medium hover:bg-brand-dark transition-colors"
                  >
                    Open Net Price Calculator →
                  </a>
                )}
              </div>
            </div>

            <div className="flex items-start gap-4 bg-white/10 rounded-xl p-4">
              <div className="bg-brand text-ink rounded-full w-8 h-8 flex items-center justify-center font-bold flex-shrink-0">2</div>
              <div>
                <h3 className="font-semibold">File Your FAFSA Early</h3>
                <p className="text-white/80 text-sm mt-1">
                  The Free Application for Federal Student Aid opens October 1st each year. Filing early gives you the best
                  chance at aid. It&apos;s free and required for most financial aid.
                </p>
                <a
                  href="https://studentaid.gov/h/apply-for-aid/fafsa"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block mt-2 bg-brand text-ink px-3 py-1 rounded-lg text-sm font-medium hover:bg-brand-dark transition-colors"
                >
                  Go to FAFSA →
                </a>
              </div>
            </div>

            <div className="flex items-start gap-4 bg-white/10 rounded-xl p-4">
              <div className="bg-brand text-ink rounded-full w-8 h-8 flex items-center justify-center font-bold flex-shrink-0">3</div>
              <div>
                <h3 className="font-semibold">Plan for Your Gap</h3>
                <p className="text-white/80 text-sm mt-1">
                  {currentGap !== null && currentGap <= 0
                    ? "Great news - this school may cost you nothing or even provide extra funds! Still, having a small savings cushion helps with unexpected expenses."
                    : `You'll need to cover about ${formatMoney(fourYearTrueCost)} over 4 years. Options include: savings, work-study jobs, payment plans, or carefully considered loans as a last resort.`
                  }
                </p>
              </div>
            </div>

            <div className="flex items-start gap-4 bg-white/10 rounded-xl p-4">
              <div className="bg-brand text-ink rounded-full w-8 h-8 flex items-center justify-center font-bold flex-shrink-0">4</div>
              <div>
                <h3 className="font-semibold">Get Free Guidance from Crown Roots Foundation</h3>
                <p className="text-white/80 text-sm mt-1">
                  We&apos;re here to help Pocono families navigate the college process. From FAFSA help to application essays,
                  our team can guide you through every step — at no cost.
                </p>
                <a
                  href="https://crownroots.org"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block mt-2 bg-mint text-mint-ink px-3 py-1 rounded-lg text-sm font-medium hover:bg-mint/80 transition-colors"
                >
                  Connect with Crown Roots Foundation →
                </a>
              </div>
            </div>
          </div>

          <div className="mt-6 pt-4 border-t border-white/20 text-center">
            <p className="text-white/80 text-sm">
              <strong className="text-white">Remember:</strong> College is an investment in your future.
              Don&apos;t let The Gap discourage you — with the right plan, it&apos;s achievable.
            </p>
          </div>
        </div>
      </div>

      <SiteFooter />
    </main>
  )
}
